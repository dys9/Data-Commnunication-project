...
UINT RXThread(LPVOID arg)//Receive (Rx)
{
	...
	CUDPServer_thdDlg *pDlg = (CUDPServer_thdDlg *)pArg->pDlg; //dlg포인터 pDlg생성하여 pArg->pList를 대입한다.
...
BOOL CUDPServer_thdDlg::OnInitDialog()
{
	...
	{
		pThread1 = AfxBeginThread(TXThread, (LPVOID)&arg1); //스레드를 생성시키고, 그것을 pThread1이라고한다.
		pThread2 = AfxBeginThread(RXThread, (LPVOID)&arg2); //스레드를 생성시키고, 그것을 pThread2라고 한다.
